...
\restrict mhXs7WrhCa03RBFrY7LfZtPoq6I9qr4bfMDDA7VaavugSuVnWtIKDvyUeApW0zp
...
--
-- Name: fn_normalize_iso_code(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.fn_normalize_iso_code() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: normalize_iso_code(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.normalize_iso_code() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    NEW.iso_code = CASE LOWER(REPLACE(NEW.iso_code, '-', ''))
...
CREATE TABLE public.content_unit (
    ...
    variant_confidence double precision,
...
CREATE VIEW public.v_content_full_context WITH (security_invoker='true') AS
 SELECT cu.unit_id,
    cu.content_raw,
    cu.content_type,
    cu.cefr_level,
...
COPY public.content_unit (unit_id, content_raw, content_type, content_hash, cefr_level, is_idiom, difficulty, variant_confidence, source_origin, created_at, syntax_ud_json, tech_meta_json, source_metadata_json, variant_id) FROM stdin;
\.
...
COPY public.vector_index (vector_id, pinecone_id, embedding_model, vector_status, indexed_at, model_id, unit_id) FROM stdin;
\.
...
SELECT pg_catalog.setval('public.language_variant_variant_id_seq', 2, true);
...
--
-- Name: vector_index vector_index_model_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
\unrestrict mhXs7WrhCa03RBFrY7LfZtPoq6I9qr4bfMDDA7VaavugSuVnWtIKDvyUeApW0zp
